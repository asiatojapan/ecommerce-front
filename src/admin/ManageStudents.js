...
function GlobalFilter({
  ...
  <legend class="pa0 f5 f4-ns mb3 black-80">Search Students</legend>
    <div class="cf">
    <input
      value={globalFilter || ''}
      onChange={e => {
...
export const Table = function ({ columns, data, userid }) {
        ...
        {page.map(
          (row, i) => {
            ...
            return (
              <tr {...row.getRowProps()}>
                {row.cells.map(cell => {
                  return <td class="pr3 pb2 bb b--black-20" {...cell.getCellProps()}>{cell.render('Cell')}</td>
                })}
              </tr>
            )}
        )}
...
const ManageStudent = () => {
 ...
 const data = students
  ...
  <div class="cf">
    ...
    <legend class="fw7 f3">Students</legend>
    </div>
    <div class="fl w-100 w-25-ns pa2 tr">
      <Link to={`/admin/create/student`} className="f6 link dim br2 ph3 pv2 mb2 dib white bg-near-black">+ Add Students </Link>
    </div>
  </div>
